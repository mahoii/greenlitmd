...
- **Env vars optional:** `ANTHROPIC_MODEL` (overrides the hardcoded `claude-sonnet-4-6` model string in `lib/anthropic.ts`), `NEXT_PUBLIC_APP_URL` (defaults to `https://orthren.com`, used in admin launch-email links and unsubscribe tokens), `NEXT_PUBLIC_POSTHOG_KEY` / `NEXT_PUBLIC_POSTHOG_HOST` (client-side PostHog init, `app/providers.tsx`), `SUPABASE_URL` (legacy fallback checked before `NEXT_PUBLIC_SUPABASE_URL` in `lib/supabase/server.ts`'s service-role client), `PA_HASH_SALT` (HMAC salt for `lib/hash-patient-name.ts`; required for Team-tier usage metering — `hashPatientName` throws without it, which the metering call site in `app/api/generate-pa/route.ts` catches and logs rather than failing generation, so a missing salt silently disables metering rather than crashing the build; must be set in production for `/billing` to have real data — under per-packet pricing (`lib/pricing.ts`), a missing salt is now a **revenue** gap, not just a reporting one), `NEXT_PUBLIC_STRIPE_LINK_PLATFORM` (Stripe Payment Link URL for the $500/mo platform fee on `/pricing`, `lib/pricing.ts`; per-packet case charges are invoiced separately, not through this link)
